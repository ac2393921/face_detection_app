...
smlie_cascade = cv2.CascadeClassifier('frecog/haarcascade_smile.xml')
face_cascade = cv2.CascadeClassifier('frecog/haarcascade_frontalface_default.xml')
...
def detect_smiles(our_image):
    ...
    gray_img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    # Detect Smiles
    smiles = smlie_cascade.detectMultiScale(gray_img, 1.1, 4)
    # Draw Rectangle
...
def cartonize_image(our_image):
    ...
    edges = cv2.adaptiveThreshold(gray_img, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY)
    # Color
...
def main():
    ...
    if choice == 'Detection':
        ...
        if image_file is not None:
            ...
            enhance_type = st.sidebar.radio('Enhance Type', ['Original', 'Gray-Scale', 'Contrast',
                                                             'Brightness', 'Blurring'])
            ...
            if enhance_type == 'Brightness':
                c_rate = st.sidebar.slider('Blurring', 0.5, 3.5)
